...
class ResidualAttentionBlock_MPL(nn.Module):
    def __init__(self, d_model: int, n_head: int, attn_mask: torch.Tensor = None, design_details=None,
                 text_layer=False, i=0):
        super().__init__()
        self.attn = nn.MultiheadAttention(d_model, n_head)
        ...
        self.text_layer = text_layer
        self.attn_mask = attn_mask
        self.txt_prompt_nctx = design_details['n_ctx']
        self.img_ntx_rm = design_details['n_rm_op']
        if i == 0:
            self.first_layer = True
        else:
            self.first_layer = False
        ...
        if not self.first_layer:
            if len(compound_prompts_deeper) > 0:
                if not self.text_layer:
                    # visual branch
                    if not (counter > len(compound_prompts_deeper) - 1):
                        # remove continuous prompts based on the remove operation
                        prefix = x[0:x.shape[0] - self.img_ntx_rm, :, :]
                        visual_context = compound_prompts_deeper[counter]  # extract the correct index
                        visual_context = visual_context.expand(x.shape[1], -1, -1).permute(1, 0, 2).half()
                        x = torch.cat([prefix, visual_context], dim=0)
                        counter += 1
                    ...
                    if not (counter > len(compound_prompts_deeper) - 1):
                        prefix = x[:1, :, :]
                        suffix = x[1 + self.txt_prompt_nctx:, :, :]
                        textual_context = compound_prompts_deeper[counter]
                        textual_context = textual_context.expand(x.shape[1], -1, -1).permute(1, 0, 2).half()
                        x = torch.cat([prefix, textual_context, suffix], dim=0)
                        counter += 1
        ...
        if current_trainer == 'IVLP' or current_trainer == 'VPT':
            ...
            self.resblocks = nn.Sequential(
                *[ResidualAttentionBlock_MaPLe(width, heads, attn_mask, design_details, text_layer, i)
                  for i in range(layers)])
        elif current_trainer == 'MPL':
            self.resblocks = nn.Sequential(
                *[ResidualAttentionBlock_MPL(width, heads, attn_mask, design_details, text_layer, i)
                  for i in range(layers)])
        else:
...
class VisionTransformer_MultiModal(nn.Module):
    ...
    def forward(self, x: torch.Tensor, shared_ctx, compound_deeper_prompts):
        ...
        # are trainable parameters here in whole image encoder.
        if self.VPT_shallow:
            # shared_ctx: (n_ctx, hidden_img_dimension)
...
class CLIP(nn.Module):
    def __init__(self,
        ...
        if isinstance(vision_layers, (tuple, list)):
            ...
            if trainer == "MaPLe" or trainer == 'MPL':
                self.visual = VisionTransformer_MultiModal(
                    input_resolution=image_resolution,
